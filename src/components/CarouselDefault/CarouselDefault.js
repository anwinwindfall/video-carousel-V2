...
const CarouselDefault = ({videos, type, responsive, heading}) => {
  ...
  const items = videos.map((items) => {
  if (type=='default') {
    return <Card key={items?.video_url} url={items?.video_url} type={type} poster={items?.thumbnail} title={items?.video_title}/>
  }
  else{
    return <CardStory key={items?.video_url} url={items?.video_url} type={type} poster={items?.thumbnail} title={items?.video_title}/>
  }  
